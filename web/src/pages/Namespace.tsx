...
export const NamespacePage: FC = () => {
    ...
    // data fetchers
    const { data: namespaceInfo, isLoading: namespaceInfoIsLoading } = useSWR(namespace ? `${namespace}-info` : null, () => namespaceFetcher(namespace || ''))
    const { data: projects, isLoading: projectsIsLoading } = useSWR(namespace ? `${namespace}-projects` : null, () => projectsFetcher(namespace || ''))
    ...
    return (
        <PageLayout title={namespace}>
          ...
          <div>
            {
              namespaceInfoIsLoading ? ( <p>Loading...</p> ) : (
                <code>
                  <pre>
                    {JSON.stringify(namespaceInfo, null, 2)}
                  </pre>
                </code>
              )
            }
          ...
          <div className="mt-3">
            {
              projectsIsLoading || projects === undefined ? ( <p>Loading...</p> ) : projects.items.map((project) => (
